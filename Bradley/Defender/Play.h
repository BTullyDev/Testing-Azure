...
// A pixel structure to represent an ARGB format pixel
struct Pixel
{
	Pixel() {}
...
public:
	...
	inline void Draw( int spriteId, Point2f pos, int frameIndex ) const { DrawTransparent( spriteId, pos, frameIndex ); }
	// Draw the sprite with transparency (slower than without transparency)
	void DrawTransparent( int spriteId, Point2f pos, int frameIndex, BlendColour globalMultiply = { 1.0f, 1.0f, 1.0f, 1.0f } ) const; // This just to force people to consider when they use an explicit alpha multiply
	// Draw the sprite rotated with transparency (slowest draw)
	void DrawRotated( int spriteId, Point2f pos, int frameIndex, float angle, float scale = 1.0f, BlendColour globalMultiply = { 1.0f, 1.0f, 1.0f, 1.0f } ) const;
...
private:
	...
	BlendMode m_blendMode{ BLEND_NORMAL };
...
void PlayGraphics::DrawTransparent( int spriteId, Point2f pos, int frameIndex, BlendColour globalMultiply ) const
{
	...
	{
		case BLEND_NORMAL:
			m_blitter.BlitPixels<AlphaBlendPolicy>( spr.preMultAlpha, frameOffset, destx, desty, spr.width, spr.height, globalMultiply );
			break;
		case BLEND_ADD:
			m_blitter.BlitPixels<AdditiveBlendPolicy>( spr.preMultAlpha, frameOffset, destx, desty, spr.width, spr.height, globalMultiply );
			break;
		case BLEND_MULTIPLY:
			m_blitter.BlitPixels<MultiplyBlendPolicy>( spr.canvasBuffer, frameOffset, destx, desty, spr.width, spr.height, globalMultiply );
			break;
		default:
...
void PlayGraphics::DrawRotated( int spriteId, Point2f pos, int frameIndex, float angle, float scale, BlendColour globalMultiply ) const
{
	const Sprite& spr = vSpriteData[spriteId];
	...
	{
		case BLEND_NORMAL:
			m_blitter.RotateScalePixels<AlphaBlendPolicy>( spr.preMultAlpha, frameOffset, destx, desty, spr.width, spr.height, spr.originX, spr.originY, angle, scale, globalMultiply );
			break;
		case BLEND_ADD:
			m_blitter.RotateScalePixels<AdditiveBlendPolicy>( spr.preMultAlpha, frameOffset, destx, desty, spr.width, spr.height, spr.originX, spr.originY, angle, scale, globalMultiply );
			break;
		case BLEND_MULTIPLY:
			m_blitter.RotateScalePixels<MultiplyBlendPolicy>( spr.canvasBuffer, frameOffset, destx, desty, spr.width, spr.height, spr.originX, spr.originY, angle, scale, globalMultiply );
			break;
		default:
			PLAY_ASSERT_MSG( false, "Unsupported blend mode in DrawRotated" )
				break;
...
void PlayGraphics::DrawPixel( Point2f pos, Pixel srcPix )
{
	switch( m_blendMode )
	{
		case BLEND_NORMAL:
			...
			m_blitter.DrawPixelPreMult<AlphaBlendPolicy>( static_cast<int>( pos.x + 0.5f ), static_cast<int>( pos.y + 0.5f ), srcPix );
			break;
		case BLEND_ADD:
			// Convert floating point co-ordinates to pixels
			m_blitter.DrawPixelPreMult<AdditiveBlendPolicy>( static_cast<int>( pos.x + 0.5f ), static_cast<int>( pos.y + 0.5f ), srcPix );
			break;
		case BLEND_MULTIPLY:
			// Convert floating point co-ordinates to pixels
			m_blitter.DrawPixel<MultiplyBlendPolicy>( static_cast<int>( pos.x + 0.5f ), static_cast<int>( pos.y + 0.5f ), srcPix );
			break;
		default:
...
void PlayGraphics::DrawLine( Point2f startPos, Point2f endPos, Pixel pix )
{
	...
	int endX = static_cast<int>( endPos.x + 0.5f );
	int endY = static_cast<int>( endPos.y + 0.5f );
...
void PlayGraphics::DrawPixelData( PixelData* pixelData, Point2f pos, float alpha )
{
	...
	m_blitter.BlitPixels<AlphaBlendPolicy>( *pixelData, 0, static_cast<int>( pos.x ), static_cast<int>( pos.y ), pixelData->width, pixelData->height, { alpha, 1.0f, 1.0f, 1.0f } );
}
...
void PlayGraphics::DrawTimingBar( Point2f pos, Point2f size )
{
	...
	{
		endPixel += static_cast<int>( ( size.width * t.millisecs ) / ( 1000.0f / FRAMES_PER_SECOND ) );
...
extern void _myFunction4isgreat();
...
namespace Play
{
	...
	void SetDrawingBlendMode( BlendMode blendMode )
	{
		PlayGraphics::Instance().SetBlendMode( static_cast<PlayGraphics::BlendMode>(blendMode) );
	}
	...
	{
		PlayGraphics::Instance().DrawTransparent( PlayGraphics::Instance().GetSpriteId( spriteName ), TRANSFORM_SPACE( pos ), frameIndex, { opacity, colour.red / 100.0f, colour.green / 100.0f, colour.blue / 100.0f }  );
	}

	void DrawSpriteTransparent( int spriteID, Point2D pos, int frameIndex, float opacity )
	{
		PlayGraphics::Instance().DrawTransparent( spriteID, TRANSFORM_SPACE( pos ), frameIndex, { opacity, 1.0f, 1.0f, 1.0f } );
	}
	...
	{
		PlayGraphics::Instance().DrawRotated( PlayGraphics::Instance().GetSpriteId( spriteName ), TRANSFORM_SPACE( pos ), frameIndex, angle, scale, { opacity, colour.red / 100.0f, colour.green / 100.0f, colour.blue / 100.0f } );
	}

	void DrawSpriteRotated( int spriteID, Point2D pos, int frameIndex, float angle, float scale, float opacity, Colour colour )
	{
		PlayGraphics::Instance().DrawRotated( spriteID, TRANSFORM_SPACE( pos ), frameIndex, angle, scale, { opacity, colour.red / 100.0f, colour.green / 100.0f, colour.blue / 100.0f } );
	}
	...
	void DrawSpriteLine( Point2f startPos, Point2f endPos, const char* penSprite, Colour c )
	{
		...
		startPos = TRANSFORM_SPACE( startPos );
	...
	{
		if( obj.type == -1 ) return; // Don't draw noObject
		PlayGraphics::Instance().DrawTransparent( obj.spriteId, TRANSFORM_SPACE( obj.pos ), obj.frame, { opacity, 1.0f, 1.0f, 1.0f } );
	}

	void DrawObjectRotated( GameObject& obj, float opacity )
	{
		if( obj.type == -1 ) return; // Don't draw noObject
		PlayGraphics::Instance().DrawRotated( obj.spriteId, TRANSFORM_SPACE( obj.pos ), obj.frame, obj.rotation, obj.scale, { opacity, 1.0f, 1.0f, 1.0f } );
	}
